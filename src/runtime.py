...
class Runtime:
    ...
    def _notify_exporters(self, scan: ScanResult|None, filament: GenericFilament|None, reader: RfidReader, events: list[ExporterEvent]):
        for exporter in self.exporters:
            if exporter.has_events(events):
        ...
        while True:
            for i, reader in enumerate(self.rfid_readers):
                ...
                if filament:
                    ...
                    self._notify_exporters(scan_result, filament, reader, [ExporterEvent.TAG_READ, ExporterEvent.TAG_DETECTED])
                    ...
                    self._notify_exporters(scan_result, None, reader, [ExporterEvent.TAG_DETECTED])
                    self.read_retries_left[i] = 0
